...
const Answers = ({ answers, onAnswerSelect, selectedAnswer, showResult }: AnswersProps) => {
    const answerItems = [
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 w-full max-w-3xl mx-auto mt-4 md:mt-6">
        {answers.map((answer, index) => {
          ...
          if (showResult) {
            ...
            } else if (selectedAnswer && selectedAnswer.text === answer.text && !answer.isCorrect) {
              buttonClass = `bg-red-500 text-white text-base md:text-lg font-semibold py-4 md:py-5 rounded-lg shadow-md`;
          ...
          return (
            <Button
              ...
              disabled={showResult}
              aria-label={`Answer ${index + 1}: ${answer.text}${showResult ? (answer.isCorrect ? ' (Correct)' : (selectedAnswer && selectedAnswer.text === answer.text ? ' (Incorrect)' : '')) : ''}`}
            >
